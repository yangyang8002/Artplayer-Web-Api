...
class PluginManager {
    constructor({ app, store, model, readConfig, saveConfig, restartServer, log, version }) {
        this.app = app;
        this.store = store;
        this.model = model;
    ...
    async install({ pkg, version }) {
        ...
        if (!fs.existsSync(PLUGIN_DIR)) fs.mkdirSync(PLUGIN_DIR, { recursive: true });
        try {
            execSync('npm install --prefix "' + PLUGIN_DIR + '" "' + spec + '" --no-audit --no-fund', { stdio: 'pipe', timeout: 300000, maxBuffer: 10 * 1024 * 1024 });
        } catch (e) {
    ...
    async uninstall(name) {
        ...
        try {
            execSync('npm uninstall --prefix "' + PLUGIN_DIR + '" "' + (meta.source.pkg || name) + '"', { stdio: 'pipe', timeout: 120000 });
        } catch (e) {}
    ...
    async update(name) {
        ...
        try {
            execSync('npm install --prefix "' + PLUGIN_DIR + '" "' + meta.source.pkg + '@latest" --no-audit --no-fund', { stdio: 'pipe', timeout: 300000 });
        } catch (e) {
