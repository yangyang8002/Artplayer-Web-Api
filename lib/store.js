...
async function collectAll(store) {
    const [danmu, videos, banned_words, accounts, security, login_logs, login_fails, api_stats, ip_stats, subtitles, video_subs] = await Promise.all([
        ...
        store.subtitleAll(), store.videoSubsAll()
    ]);
    return { danmu, videos, banned_words, accounts, security, login_logs, login_fails, kv: { api_stats, ip_stats }, subtitles, video_subs };
}
